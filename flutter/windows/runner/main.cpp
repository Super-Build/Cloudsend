...
typedef char** (*FUNC_CLOUDSEND_CORE_MAIN)(int*);
typedef void (*FUNC_CLOUDSEND_FREE_ARGS)(char**, int);
typedef int (*FUNC_CLOUDSEND_GET_APP_NAME)(wchar_t*, int);
...
{
  HINSTANCE hInstance = LoadLibraryA("cloudsend.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load cloudsend.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_CLOUDSEND_CORE_MAIN cloudsend_core_main =
      (FUNC_CLOUDSEND_CORE_MAIN)GetProcAddress(hInstance, "cloudsend_core_main_args");
  if (!cloudsend_core_main)
  {
    std::cout << "Failed to get cloudsend_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_CLOUDSEND_FREE_ARGS free_c_args =
      (FUNC_CLOUDSEND_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = cloudsend_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "CloudSend [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"CloudSend";
  FUNC_CLOUDSEND_GET_APP_NAME get_cloudsend_app_name = (FUNC_CLOUDSEND_GET_APP_NAME)GetProcAddress(hInstance, "get_cloudsend_app_name");
  if (get_cloudsend_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_cloudsend_app_name(app_name_buffer, 512) == 0) {
