...
- Legacy `init_service` and legacy Flutter `start_capture` are non-authorizing compatibility entries. They must never request `MediaProjection`; only explicit Android UI `start_screen_share` and side-button `开共享` / `start_capture2` may do so.
- Repeated `start_screen_share` while `_isStart == true` is a no-op. Legacy `start_capture` may only refresh an already-active normal video path and return the current state; it must not call `restoreMediaProjection()`, reuse an Android 14+ token, or reopen the screen-share authorization dialog.
- `restoreMediaProjection(...)` defaults to `allowPermissionPrompt = false`. Hidden PC connect/reconnect/first-frame paths may reuse an existing live/saved projection when possible, but they must not open a new Android screen-share prompt. Only explicit Android UI start or side-button `开共享` may pass `allowPermissionPrompt = true`.
- `updateScreenInfo(...)` must not call `stopCapture()` + `startCapture()` while normal screen sharing is active. Screen metric changes may resize/rebind the existing `VirtualDisplay` surface, but must not release the active `MediaProjection` token or reopen authorization.
- Remote `start_capture2` open/close commands received during the short authorized-connection settle window are ignored only when a live/starting/in-flight screen share already exists. This protects PC first-connect from accidentally closing/reopening an already authorized projection while keeping normal side-button operations available after the settle window.
- Screen-share permission in-flight only blocks duplicate prompts while the Android authorization dialog is active. It must not add a post-result cooldown; explicit close-share then open-share should be allowed immediately.
...
- Android 授权 `add_connection` 且正常屏幕共享已完整开启时，会通过 `forceVideoFrameRefresh(...)` 补正常视频首帧；PC waiting 状态也可补发正常 `sessionRefreshVideo(...)` 请求。二者都只能刷新正常视频，不能在共享未完整活跃时启动 Rust video refresh，不能自动切无视/截屏 fallback，也不能重绑 `VirtualDisplay` surface 或重新请求屏幕共享授权。
- `startCapture()` has a short `captureStarting` first-frame acceptance window while `VirtualDisplay` is being created. It protects fast initial `ImageReader` frames on static Huawei Android 10-13 screens; all stop/failure/projection-loss paths must clear it.
