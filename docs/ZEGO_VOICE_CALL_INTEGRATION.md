...
- `flutter/lib/models/server_model.dart::_hasLocalAndroidVoiceCall` is local to one Android device/process. It rejects another client's pending/active ZEGO call or the current client's active ZEGO call to preserve strict 1v1 audio, but it clears stale local `ZegoVoiceCallModel.active` when the only current signal is a new incoming invite.
