...
- Current source truth: `createOrSetVirtualDisplay(...)` still requests new `MediaProjection` permission after `SecurityException`; `BootReceiver.kt` still uses `ACT_INIT_MEDIA_PROJECTION_AND_SERVICE`. Treat any docs claiming no-prompt SecurityException handling or boot core-only startup as reverted/stale unless code changes again.
